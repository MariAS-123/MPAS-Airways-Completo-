...
public class ExceptionHandlingMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        catch (RpcException ex)
        {
            _logger.LogError(
                ex,
                "Error gRPC al comunicarse con MS Vuelos. StatusCode={GrpcStatusCode} Detail={GrpcDetail} TraceId={TraceId}",
                ex.StatusCode,
                ex.Status.Detail,
                context.TraceIdentifier);
            ...
            await WriteErrorResponseAsync(
                ...
                ApiErrorResponse.Create(
                    "No se pudo comunicar con MS Vuelos via gRPC.",
                    new[]
                    {
                        $"GrpcStatusCode: {ex.StatusCode}",
                        $"GrpcDetail: {ex.Status.Detail}",
                        "Verifique Integrations:Vuelos:GrpcUrl y que grpcvuelos esté desplegado con HTTP/2."
                    },
                    context.TraceIdentifier));
        }
        catch (Exception ex)
