...
namespace Microservicio.ReservasF.DataAccess.Repositories
{
        ...
        : IFacturaRepository
    {
        ...
        public async Task<(List<FacturaEntity> Items, int Total)> ObtenerPagedAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!string.IsNullOrWhiteSpace(numeroFactura))
                query = query.Where(f => f.NumeroFactura.Contains(
                    numeroFactura.Trim().ToUpperInvariant()));

            if (!string.IsNullOrWhiteSpace(estado))
                query = query.Where(f => f.Estado == estado.Trim().ToUpperInvariant());
