...
Igual que Vue: pantalla de login solo pide **usuario y contraseña** (botón «Iniciar Sesión» en el header).

Para **reservar**, el usuario debe tener rol **CLIENTE**. El `idCliente` se obtiene así:

1. Claim `id_cliente` del JWT (Seguridad lo agrega si `usuario_app.id_cliente` está vinculado).
2. Si falta en el token, query `miPerfilCliente` (proxy a `GET /clientes/portal/mi-perfil`).

`admin` / `admin123` sirve para probar navegación, pero no trae `id_cliente` — la reserva fallará en checkout, igual que en la web.
...
Guardar en contexto/AsyncStorage: `token`, `roles`, `idCliente` (resuelto automáticamente, no manual).

### Perfil cliente (post-login)

```graphql
query MiPerfilCliente {
  miPerfilCliente
}
```

Requiere `Authorization: Bearer`. Devuelve JSON con `idCliente` / `id_cliente`.
...
```graphql
query BuscarVuelos($origen: String, $destino: String, $fecha: String) {
  buscarVuelos(origen: $origen, destino: $destino, fecha: $fecha)
}
```

Variables (mismo formato que la web REST):
...
**Importante — error típico Apollo:** si la query declara `$fecha: Date` pero envían un `Date` de JavaScript, o usan `$fecha: String` contra un schema `Date`, aparece:

```text
The variable `fecha` is not compatible with the type of the current location.
```

Solución: **`$fecha: String`** y valor **`"2026-06-15"`** (texto, no objeto `Date`).

**No existe “listar todos los vuelos”** sin origen/destino/fecha (igual que Vue/Middleware). La pantalla de resultados debe recibir esos 3 datos del formulario de búsqueda anterior.
...
```json
{
  "input": {
    ...
    "idCliente": "<id del cliente logueado>",
    "idVuelo": 25,
    "idAsiento": 4033
  }
}
