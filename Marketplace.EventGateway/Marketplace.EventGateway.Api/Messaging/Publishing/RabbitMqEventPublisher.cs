...
public sealed class RabbitMqEventPublisher : IMarketplaceEventPublisher, IAsyncDisposable
{
        ...
        where TEvent : MarketplaceEvent
    {
        if (!_options.Enabled)
        {
            _logger.LogWarning("RabbitMQ deshabilitado. Evento {EventType} no publicado.", typeof(TEvent).Name);
            return;
        }
        ...
        _logger.LogInformation(
            "Gateway publicó {EventType} routingKey={RoutingKey} correlationId={CorrelationId}",
            typeof(TEvent).Name,
            routingKey,
